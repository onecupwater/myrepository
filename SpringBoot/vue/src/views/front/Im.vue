            ...
            <el-col :span="4" style="text-align: center">
                <el-card style="width: 300px; height: 300px; color: #333">
                    <div style="padding-bottom: 10px; border-bottom: 1px solid #ccc">
                        在线用户列表<span style="font-size: 12px">（点击聊天气泡开始聊天）</span>
                    </div>
                    <div style="padding: 10px 0" v-for="item in users" :key="item.username">
                        <span>{{ item.username }}</span>
                        <i class="el-icon-chat-dot-round" style="margin-left: 10px; font-size: 16px; cursor: pointer"
                           @click="chatUser = item.username"></i>
                        <span style="font-size: 12px;color: limegreen; margin-left: 5px" v-if="item.username === chatUser">chatting...</span>
                    </div>
                </el-card>
            </el-col>
            <!--聊天输入框-->
            <el-col :span="20">
                <div style="width: 800px; margin: 0 auto; background-color: white;
                    ...
                    <div style="text-align: center; line-height: 35px;">
                        <div style="font-size: 25px">Web聊天室</div>
                        <div v-if="chatUser" style="color: #8c939d">正在和（{{ chatUser }}）聊天</div>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            init(){
                ...
                let _this = this; //这个很重要，要是没有这个，在onmessage里this.变量名都会失效
                if (typeof (WebSocket) == "undefined") {
                    console.log("您的浏览器不支持WebSocket");
                } else {
                    console.log("您的浏览器支持WebSocket");
                    ...
                    //  浏览器端收消息，获得从服务端发送过来的文本消息(在线用户列表的json)
