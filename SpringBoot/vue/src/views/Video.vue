    ...
    <div>
        <div style="margin: 15px 0px">
            ...
            <el-popconfirm
                    ...
                    icon="el-icon-info"
                    icon-color="red"
                    title="只能删除视频记录，无法删除视频本身，您确定批量删除视频吗？"
        ...
        <el-table :data="tableData" border stripe header-cell-class-name="headerCellClass"
            ...
            <el-table-column align="center" label="封面">
                    <template slot-scope="scope">
                        <el-button type="primary" v-if="isShowCoverButton(scope.row.cover)"
                                   @click="previewCover(scope.row.cover)">预览封面</el-button>
                    </template>
            </el-table-column>
            <el-table-column align="center" label="视频">
                <template slot-scope="scope">
                    <el-button type="primary" @click="previewVideo(scope.row.url)">预览视频</el-button>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" width="180px">
                <template slot-scope="scope">
                    ...
                    <el-popconfirm
                            ...
                            icon="el-icon-info"
                            icon-color="red"
                            title="只能删除视频记录，无法删除视频本身，您确定删除吗？"
            ...
            <el-form :model="form" label-width="70px">
                ...
                <el-form-item label="封面">
                    <el-upload
                            ...
                            :show-file-list="false"
                            :on-success="handleAfterUploadCover">
...
<script>
    export default {
        ...
        methods: {
            ...
            handleAfterUploadCover(response){
                if(response.code!==200){
                    this.$message.error(response.msg);
                }else {
            ...
            handleEdit(row){
                ...
                this.form = row;
                this.coverUploadAction = "http://localhost:9090/video/uploadCover/" + row.id;
            },
            handleAdd(){
            ...
            isShowCoverButton(cover){
                return cover !== null;
            },
            previewCover(cover){
                window.open('https://file.keking.cn/onlinePreview?url='
                    +encodeURIComponent(window.btoa(cover)));
            },
            previewVideo(url){
                window.open('https://file.keking.cn/onlinePreview?url='
