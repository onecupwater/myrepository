<template>
    <div style="margin-top: 30px">
        <el-button type="primary" style="width: 205px; height: 40px" @click="returnCourseList">返回任课列表</el-button>
        <div style="margin-top: 30px">
                ...
                <el-table-column align="center" label="操作">
                    <template slot-scope="scope">
                        <el-popconfirm
                                ...
                                icon-color="red"
                                title="您确定取消任课吗？"
                                @confirm="deleteCourseIdAndTeacherId(scope.row.id)"
                        >
                            <el-button type="danger" plain icon="el-icon-delete" slot="reference">取消任课</el-button>
                        </el-popconfirm>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    export default {
        ...
        methods :{
            load(){
                this.request.get("user/showTeacherCourse/"+this.user.id).then(res=>{
                    this.courses = res.data;
                });
            },
            deleteCourseIdAndTeacherId(courseId){
                this.request.delete("course/deleteCourseIdAndTeacherId/"+this.user.id+"/"+courseId)
                    .then(res=>{
                    if(res.code===200){
